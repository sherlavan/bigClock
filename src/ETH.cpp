...
bool ETHClass::beginSPI(eth_phy_type_t type, uint8_t phy_addr, int cs, int irq, int rst, SPIClass *spi, uint8_t spi_freq_mhz){
    ...
    uint8_t mac_addr[6];
    base_mac_addr[6 - 1] += _eth_index; //Increment by the ETH number
...
bool ETHClass::begin(eth_phy_type_t type, uint8_t phy_addr, int cs, int irq, int rst, SPIClass &spi, uint8_t spi_freq_mhz){
    ...
    return beginSPI(type, phy_addr, cs, irq, rst, &spi, spi_freq_mhz);
}
#endif
...
//     return beginSPI(type, phy_addr, cs, irq, rst, NULL, spi_freq_mhz);
// }
